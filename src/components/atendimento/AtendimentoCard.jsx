  ...
  return (
    <div className="flex items-center gap-2 text-sm overflow-hidden">
      ...
      <div className="min-w-0 flex flex-wrap items-baseline gap-x-1">
        ...
        <span className="font-medium truncate text-sm tracking-widest max-w-46">{value}</span>
      </div>
    </div>
  );
}
...
export function AtendimentoCard({ atendimento, now, aguardandoAssociado }) {
  ...
  const empresa = normalizar(atendimento.empresa);

  // Card para quando o agente já respondeu e aguarda o cliente
  if (aguardandoAssociado) {
    return (
      <div className="group relative rounded-lg p-4 flex flex-col gap-3 bg-white text-slate-900 border border-slate-200 shadow-sm hover:shadow-md transition-all">
        ...
        <div className="flex items-center justify-between mt-1 pt-3 border-t border-slate-100">
          <span className="text-lg font-bold text-slate-400 uppercase tracking-widest truncate max-w-46">
            {empresa}
          </span>
  ...
  return (
    <div
      className={`group relative rounded-lg p-4 flex flex-col gap-3 text-white overflow-hidden shadow-lg ${s.bg} transition-all duration-500`}
    >
      ...
      <div className="relative flex items-center justify-between mt-1 pt-3 border-t border-white/20">
        <span className="text-lg font-bold text-white/90 uppercase tracking-widest truncate max-w-42">
          {empresa}
        </span>
